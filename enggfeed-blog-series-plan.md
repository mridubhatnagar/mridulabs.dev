...
**Schema diagram**: DBML for dbdiagram.io, 13 tables total (10 live tables plus `user`, `allowed_users`, and `blog_chunk` shown as a distinct dropped group, rather than the 11 tables the `initial` migration alone created — `feedback` and `llm_usage` came later and are included too), grouped by concern (Ingest: `blog_source`, `blog` · LLM-generated content: `summary`, `simplify`, `prerequisite`, `blog_prerequisite`, `tag`, `blog_tag` · Supporting: `feedback`, `llm_usage` · Dropped: `user`, `allowed_users`, `blog_chunk`), so the diagram carries the "started broad, cut down" story visually. Per-table `Note`s do the "what is this" explaining so the video doesn't have to. Columns verified against the actual model files (`blog/models.py`, `feedback/models.py`, `ingest/models.py`, `prerequisites/models.py`, `simplify/models.py`, `summary/models.py`, `tags/models.py`), current live schema is 10 tables. A separate 10-table-only DBML variant is used for the blog's embedded static image, since the prose already narrates the dropped-tables story in detail; the full 13-table version is used for the video walkthrough instead.
